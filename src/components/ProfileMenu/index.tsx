...
export default function ProfileMenu(): JSX.Element {
  ...
  return (
    <div className='profile-menu' data-testid='profile-menu'>
      {profile || loading ? (
        <div className={`profile-image-container ${loading ? 'loading' : ''}`} onClick={() => {profile && handleToggleProfile()}}>
        ...
        <Button text='Login' onClick={() => setShowLogin(true)} data-testid='profile-login' />
      )}      
      <SidePanel show={showLogin} onClose={() => setShowLogin(false)}>
        ...
        <div className='form'>
          ...
          <Button text='Login' onClick={handleLogin} color={ButtonColors.accent} />
        </div>
      </SidePanel>
      <SidePanel show={showProfile} onClose={() => setShowProfile(false)}>
        ...
        <div className='profile'>
          <div className='profile-group'>
            <img src={profile?.profileImage || ''} alt='profile' className='profile-image-large'/>
            <h1>{profile?.name || 'Lorem Ipsum'}</h1>
          </div>
          <p>{profile?.email || 'Lorem Ipsum'}</p>
          <Button text='Logout' onClick={handleLogout} color={ButtonColors.accent} />
        </div>
      </SidePanel>
    </div>
  )
