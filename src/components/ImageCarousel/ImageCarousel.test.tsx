...
describe('ImageCarousel component tests', () => {
  ...
  it('should display the images in a working carousel', () => {
    render(<ImagesCarousel images={['image1.jpg', 'image2.jpg', 'image3.jpg']} />);
    expect(screen.getByTestId('carousel')).toBeInTheDocument();
    ...
    expect(screen.getByTestId('carousel-img')).toHaveAttribute('src', 'image1.jpg');    
    userEvent.click(screen.getByTestId('carousel-next'));
    expect(screen.getByTestId('carousel-img')).toHaveAttribute('src', 'image2.jpg');
    userEvent.click(screen.getByTestId('carousel-next'));
    expect(screen.getByTestId('carousel-img')).toHaveAttribute('src', 'image3.jpg');
    userEvent.click(screen.getByTestId('carousel-next'));
    expect(screen.getByTestId('carousel-img')).toHaveAttribute('src', 'image1.jpg');
